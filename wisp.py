...
PROMPT = (
    ...
    "- 'delete old compiled app files so the project can be rebuilt cleanly'\n"
    "- 'download all the libraries this project needs in order to run'\n"
    "- 'search wisp.py for where the API key is loaded, to understand how credentials are set up'\n"
    "- 'run the test suite to check whether the recent changes broke anything'\n"
    "- 'save a snapshot of the current changes with the message: fix Gemini timeout'\n"
